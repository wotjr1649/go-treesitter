...
var oracleCandidate = flag.Bool("oracle-candidate", false, "compare the authorized isolated scanner candidate")

func TestOracleExperiment(t *testing.T) {
	runOracleRecords(t, *oracleCandidate)
}

func TestOracleCandidateEdits(t *testing.T) {
	if !*oracleCandidate {
		t.Fatal("candidate edits require explicit candidate mode")
	}
		...
		for _, f := range regressionFixtures[3:5] {
			t.Run(language.name+"/"+f.id, func(t *testing.T) {
				...
				if nodeDigest(inc.Tree.Nodes()) != c.NodesSHA256 {
					t.Fatal("candidate incremental C tree difference")
				}
